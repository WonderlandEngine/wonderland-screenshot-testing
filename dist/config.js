    ...
    async validate() {
        for (const { name, scenarios } of this.projects) {
            ...
            if (missingEventScenarios.length > 0) {
                throw new Error(`'${name}': Missing 'event' or 'readyEvent' key for scenarios: ${missingEventScenarios}`);
            }
            ...
            if (!errors.length)
                continue;
            throw new Error(`'${name}' contains a scenario(s) with missing reference folder: ${errors}`);
        }
    }
}
